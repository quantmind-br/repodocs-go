...
lint: ## Run linters
	...
	@which golangci-lint > /dev/null || (echo "Installing golangci-lint v2..." && curl -sSfL https://raw.githubusercontent.com/golangci/golangci-lint/master/install.sh | sh -s -- -b $(shell go env GOPATH)/bin v2.0.0)
	golangci-lint run ./...
